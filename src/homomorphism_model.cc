    ...
    target_link_count(target.get_no_link_nodes())
{
    ...
    for (auto e = pattern.begin_edges(), e_end = pattern.end_edges() ; e != e_end ; ++e) {
        if (e->first.first == e->first.second)
            _imp->pattern_loops[e->first.first] = 1;
        else
            _imp->pattern_graph_rows[e->first.first * max_graphs + 0].set(e->first.second);
    }
    ...
    if (params.bigraph) {
        ...
        // TODO: Is this what is slowing everything down? Should I use memoized data? Will that help?
